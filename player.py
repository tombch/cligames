...
class Player:
    ...
    def minimax(self, board, current_depth, max_depth, alpha, beta):
        ...
        if (current_depth == max_depth) or (board.board_full() == True) or board.check_for_win(self.disc) or board.check_for_win(self.opponent_disc):
            board_score += board.player_score(1, 100, 100000, self.disc) # player evaluation (positive)    
            board_score -= board.player_score(10, 1000, 1000000, self.opponent_disc) # opponent evaluation (negative)
            return board_score
        if current_depth == 0:
